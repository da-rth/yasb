...
use windows::Win32::UI::Shell::{SHQueryUserNotificationState, QUNS_RUNNING_D3D_FULL_SCREEN, QUNS_PRESENTATION_MODE, QUNS_BUSY};
use windows::Win32::UI::WindowsAndMessaging::{GetWindowRect, WindowFromPoint, GetClassNameW};
    ...
    loop {
      ...
      for (label, window) in app_handle.windows() {
        ...
        if !is_fullscreen_present() || window.hwnd().is_err() || window.current_monitor().is_err() {
          if !is_visible {
            app_handle.emit_to(label.as_str(), "FullscreenShowWindow", true).unwrap();
          }
          continue;
        }
        ...
        unsafe {
          if is_monitor_fullscreen(window.hwnd().unwrap().clone()) {
            if is_visible {
              app_handle.emit_to(label.as_str(), "FullscreenHideWindow", true).unwrap();
            }
          } else {
            if !is_visible {
              app_handle.emit_to(label.as_str(), "FullscreenShowWindow", true).unwrap();
